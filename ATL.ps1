...
###SCCM###
$SiteCode = "ALN"
$SiteServer = "BBCMPS01.bakerbotts.net"

#User
$User2 = (get-wmiobject Win32_ComputerSystem).UserName.Split('\')[1]
$sccmA = Get-WmiObject -Namespace root\sms\site_$SiteCode -ComputerName $SiteServer -Query "select * from SMS_UserMachineRelationship WHERE UniqueUserName like'%$User2%' AND IsActive = '1' AND Types = '1'"
$SccmDevice = $sccmA.ResourceName
$SccmDevice1 = $sccmA.ResourceName
$SccmDevice2 = $sccmA.ResourceName

$sccmA.UniqueUserName
$c = $env:COMPUTERNAME
if($SccmDevice -match $c){
Write-Verbose "Computer Name Match in SCCM" -Verbose
}
#Device
$Device = $env:COMPUTERNAME
$SccmB = Get-WmiObject -Namespace root\sms\site_$SiteCode -ComputerName $SiteServer -Query "select * from SMS_UserMachineRelationship WHERE ResourceName like '%$Device%' AND IsActive = '1' AND Types = '1'"
$SccmB.ResourceName
$SccmUser = $SccmB.UniqueUserName
 
 ###
 ###
 ###
$A1= $SccmB.UniqueUserName
$A2= $sccmA.ResourceName
$A3= $SccmB.UniqueUserName
$A4= $sccmA.ResourceName
...
"
$PrimaryComputerreport += $datarow2
}
#################################
#################################
#---Program Check----
$HTMLApplication_Check = 'Cisco Anyconnect Secure Mobility Client','Cisco UC Integration*','Remote Support Jump Client*'
foreach($app in $HTMLApplication_Check){
$a = Get-ChildItem -Path HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall, HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall | Get-ItemProperty | Where-Object {$_.DisplayName -match $app}
$a.DisplayName
#----APP 1 ---------
if($a.DisplayName-like 'Cisco Anyconnect*'){
Write-verbose "Cisco Anyconnect Secure Mobility Client FOUND!" -Verbose
$CISCOVPN_Status = "Installed"
$ColorA = 'color:#0a9c00' #Green
} Elseif(!$a.DisplayName-like 'Cisco Anyconnect*'){
$CISCOVPN_Status = "NOT Installed"
$ColorA = 'color:#ff0000' #RED
}#End - App
#----APP 2 ---------
if($a.DisplayName-like 'Cisco UC Integration*'){
Write-verbose "Cisco UC Integration FOUND!" -Verbose
$CUCILync_Status = "Installed"
$ColorB = 'color:#0a9c00' #Green
} Elseif(!$a.DisplayName-like 'Cisco UC Integration*'){
$CUCILync_Status = "NOT Installed"
$ColorB = 'color:#ff0000' #RED
}#End - App
#----APP 3 ---------

if($a.DisplayName-like 'Remote Support Jump Client*'){
Write-verbose "Remote Support Jump Client FOUND!" -Verbose
$BTJump_Status = "Installed"
$ColorC = 'color:#0a9c00' #Green
} Elseif(!$a.DisplayName-like 'Remote Support Jump Client*'){
$BTJump_Status = "NOT Installed"
$ColorC = 'color:#ff0000' #RED
}#End - App

#
}#End

#---Task and or Application
$TaskA = "CUCILync"
$TaskB = "Cisco VPN"
$TaskC = "Task C"
$TaskD = "Task D"
#--Status
$StatusA = "$CUCILync_Status"
$StatusB = "$CISCOVPN_Status"
$StatusC = "$BTJump_Status"
$StatusD = ""
#--------
$dataRow3 = "
</tr>
<td>$TaskA </td>
<td style=$ColorA>$StatusA</td>
</tr>
<td>$TaskB </td>
<td style=$ColorB>$StatusB</td>
</tr>
<td>$TaskC </td>
<td style=$ColorC>$StatusC</td>
</tr>
<td>$TaskD </td>
<td>$StatusD</td>
</tr>
...
"
$Applicationreport += $datarow3
#################################

$report = "<html>
    <head>
        <title>ADVISOR GROUP IT Dept</title>
    </head>
<style>
...
<h2>PC Report for BB Austin Office</h2>
<table>
<tr>
<th>SCCM Listed Primary User For Device: $Device</th>
<tr>
<br>
$PrimaryUserreport
<tr>
<table>
<tr>
<th>SCCM Listed $User2 as Primary User on these Devices:</th>
<tr>
<br>
$PrimaryComputerreport
<tr>
<br/>
<tr>
<table>
...
$report| Out-File "C:\Users\$User2\AppData\Local\Temp\Index2.html"
Start-Sleep -Seconds 1
[system.Diagnostics.Process]::Start("msedge","C:\Users\$User2\AppData\Local\Temp\Index2.html")
#Set-Content -Path "C:\Windows\Temp\Index1.html" -Value $htmlexport -Force
